            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="table-header">
                        配置列表
                        <?php if(authCheck('Admin/Config/add',session(C('ADMIN_INFO'))['id'])): ?><a href="<?php echo U('Config/add');?>">
                            ...
                            <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td>
                                        <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                            ...
                                            <?php if(authCheck('Admin/Config/edit',session(C('ADMIN_INFO'))['id'])): ?><a class="green" href="<?php echo U('Config/edit',array('id'=>$vo['id']));?>"
                                                   title="修改">
                                                    <i class="icon-pencil bigger-130"></i>
                                                </a><?php endif; ?>

                                            <?php if(authCheck('Admin/Config/del',session(C('ADMIN_INFO'))['id'])): ?><a class="red" onclick="delMenu('<?php echo ($vo["id"]); ?>','<?php echo ($vo["k"]); ?>')" title="删除">
...
<script type="text/javascript">
    ...
    function delMenu(id, title) {
        layer.confirm('确定要删除配置项[' + title + ']吗？', {
            ...
            $.post("<?php echo U('Config/del');?>", {'id': id}, function (res) {
