            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="table-header">
                        管理员列表
                        <?php if(authCheck('Admin/Admin/add',session(C('ADMIN_INFO'))['id'])): ?><a href="<?php echo U('Admin/add');?>">
                    ...
                    <div class="table-responsive">
                        <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                            ...
                            <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                    ...
                                    <td>
                                        <?php if($vo["username"] != 'admin'): if(authCheck('Admin/Admin/change',session(C('ADMIN_INFO'))['id'])): if($vo["status"] == 1): ?><a href="<?php echo U('Admin/change',array('id'=>$vo['id'],'status'=>0));?>"
                                                       class="layui-btn layui-btn-mini layui-btn-primary">禁用</a><?php endif; ?>
                                                <?php if($vo["status"] == 0): ?><a href="<?php echo U('Admin/change',array('id'=>$vo['id'],'status'=>1));?>"
                                                       class="layui-btn layui-btn-mini layui-btn-primary">启用</a><?php endif; endif; ?>

                                            <?php if(authCheck('Admin/Admin/edit',session(C('ADMIN_INFO'))['id'])): ?><a href="<?php echo U('Admin/edit',array('id'=>$vo['id']));?>"
                                                   class="layui-btn layui-btn-mini layui-btn-normal">编辑</a><?php endif; ?>

                                            <?php if(authCheck('Admin/Admin/del',session(C('ADMIN_INFO'))['id'])): ?><a onclick="delMenu('<?php echo ($vo["id"]); ?>','<?php echo ($vo["username"]); ?>')"
                                                   class="layui-btn layui-btn-mini layui-btn-danger layui-btn-normal">删除</a><?php endif; endif; ?>
                                    </td>
...
<script type="text/javascript">
    ...
    function delMenu(id, name) {
        layer.confirm('确定要删除管理员[' + name + ']？', {
            ...
            $.post("<?php echo U('Admin/del');?>", {'id': id}, function (res) {
